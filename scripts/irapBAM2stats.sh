...
select count(*) as AlnSpliced from bam_index where isSpliced=1 and isDuplicate=0 and isNotPassingQualityControls=0  and isMapped=1;
select count(*) as ReadsSpliced from bam_index where isSpliced=1 and isDuplicate=0 and isNotPassingQualityControls=0  and isMapped=1 and isPrimary=1;
...
select count(*) as NM3P from bam_index where isDuplicate=0 and isNotPassingQualityControls=0  and isMapped=1 and nm>=3 ;
...
select count(*) as NHG2 from bam_index where isDuplicate=0 and isNotPassingQualityControls=0  and isMapped=1 and nh>=2 ;
select count(*) as NHG10 from bam_index where isDuplicate=0 and isNotPassingQualityControls=0  and isMapped=1 and nh>=10 ;
select count(*) as NHG20 from bam_index where isDuplicate=0 and isNotPassingQualityControls=0  and isMapped=1 and nh>=20 ;
EOF
exit 0
