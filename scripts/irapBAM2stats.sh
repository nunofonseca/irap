...
select count(*) as AlnSpliced from bam_index where nSpliced>=1 and isDuplicate=0 and isNotPassingQualityControls=0  and isMapped=1;
...
select count(*) as AlnSpliced1 from bam_index where nSpliced=1 and isDuplicate=0 and isNotPassingQualityControls=0  and isMapped=1;
select count(*) as AlnSpliced2 from bam_index where nSpliced=2 and isDuplicate=0 and isNotPassingQualityControls=0  and isMapped=1;
select count(*) as AlnSpliced3 from bam_index where nSpliced=3 and isDuplicate=0 and isNotPassingQualityControls=0  and isMapped=1;
select count(*) as AlnSplicedGE4 from bam_index where nSpliced>3 and isDuplicate=0 and isNotPassingQualityControls=0  and isMapped=1;
EOF
exit 0
