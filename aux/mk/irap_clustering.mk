...
$(quant_toplevel_folder)/sc3/genes.raw.filtered.normalised.$(quant_method).clusters.tsv: $(quant_toplevel_folder)/genes.raw.filtered.normalised.$(quant_method).$(expr_ext) $(cluster_spike_dep) 
	mkdir -p $(@D) && irap_sc3 -i $< --$(expr_format) --out $(quant_toplevel_folder)/sc3/genes.raw.filtered.$(quant_method).irap --min_clusters $(min_clusters) --max_clusters $(max_clusters) --max_threads $(max_threads) $(cluster_spike_param) && mv $(quant_toplevel_folder)/sc3/genes.raw.filtered.$(quant_method).irap_clusters.tsv $@ || ( rm -f $@* && exit 1)

$(quant_toplevel_folder)/sc3/transcripts.raw.filtered.normalised.$(quant_method).clusters.tsv: $(quant_toplevel_folder)/transcripts.raw.filtered.normalised.$(quant_method).$(expr_ext) $(cluster_spike_dep)
	mkdir -p $(@D) &&  irap_sc3 -i $< --$(expr_format) --out $(quant_toplevel_folder)/sc3/transcripts.raw.$(quant_method).irap --min_clusters $(min_clusters) --max_clusters $(max_clusters) --max_threads $(max_threads) $(cluster_spike_param) || ( rm -f $@* && exit 1)
