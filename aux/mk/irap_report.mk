...
ifeq  ($(qc),off)
$(info $(name)/report/riq/raw_data/$($(p)_dir)$(call get_fastq_prefix,$(p)).fastqc.tsv))

$(name)/report/fastq_qc_report.tsv:  $(foreach p,$(pe),$(name)/report/riq/raw_data/$($(p)_dir)/$(call get_fastq_prefix,$(p))_1.fastqc.tsv $(name)/report/riq/raw_data/$($(p)_dir)$(call get_fastq_prefix,$(p))_2.fastqc.tsv) $(foreach p,$(se),$(name)/report/riq/raw_data/$($(p)_dir)$(call get_fastq_prefix,$(p)).fastqc.tsv)
