...
function submit_job_status {
    ...
    $ECHO bsub $IRAP_LSF_PARAMS -M 1000 -R "select[mem>=1000]  rusage[mem=1000]" -q $QUEUE  -J "${jobname}n" $WAITFOR  irap_lsf_job_status.sh $jobname $JOB_ID  `get_maxmem $MEM` $IRAP_PAR_CMD
    #2> /dev/null
    if [ $? != 0 ]; then
