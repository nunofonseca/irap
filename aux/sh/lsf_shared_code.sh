...
function submit_job {
	...
	$ECHO bsub -q $QUEUE -n $THREADS  -M $MAX_MEM -R "select[mem>=$MEM] rusage[mem=$MEM]"  -w "$WAIT_FOR_IDS"  -cwd `pwd` -o "`pwd`/$name/$jobname-%J.out" -e "`pwd`/$name/$jobname-%J.err" -J $jobname  $cmd2e max_threads=$THREADS  data_dir=$DATA_DIR max_mem=$MEM
    else
	$ECHO bsub -q $QUEUE  $GROUP -n $THREADS  -M $MAX_MEM -R "select[mem>=$MEM]  rusage[mem=$MEM]"   -cwd `pwd` -o "`pwd`/$name/$jobname-%J.out" -e "`pwd`/$name/$jobname-%J.err" -J $jobname  $cmd2e max_threads=$THREADS  data_dir=$DATA_DIR max_mem=$MEM	
    fi
}
